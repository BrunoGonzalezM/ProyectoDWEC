      ...
      {error ? (<p>{error}</p>) : (
        <Box overflowX="hidden" padding="1em">
          ...
          {[
            ...
            <div key={title}>
              <Box className="tituloPelis" marginLeft="2.5em">
                {title}
              </Box>
    ...
    <>
      {ul ? (
        <Flex className="MovieList" flexDirection="row" paddingInline="0em" alignItems="center">
          <Box className="left" _hover={{ transform: "scale(1.6)", color: "white", }} height="100%" transition=".4s" color="#a7a7a7">
            <IoIosArrowBack className='left' onClick={handleMoveLeft} size={40} />
          </Box>
          <UnorderedList display="flex" listStyleType="none" overflow="hidden" padding="40px 0" flex="0 0 calc(100vw - 9em)" margin="1em" maxWidth="calc(100vw - 9em)" paddingInline="1em" ref={ref}>
            {movies.map((movie) => (
              <ListItem flex="0 0 0px" backgroundColor="#00000030" margin="0 20px 0 0" userSelect="none" cursor="pointer" transition="1s" borderRadius="5px" minH="270px" key={movie.id} _hover={{ transform: "scale(1.08)", }}>
          ...
          <Box className="right" _hover={{ transform: "scale(1.6)", color: "white", }} height="100%" transition=".4s" color="#a7a7a7">
